...
namespace DLS.ServiceTeacher
{
    public class TeacherService : TeacherProto.TeacherProtoBase
    {
        ...
        public override Task<TeacherObj> GetTeacherById(Int64Value id, ServerCallContext context)
        {
            ...
            {
                Teacher s = dbContext.Teachers.Where(x => x.Id == id.Value)
                .Include(x => x.Courses)
                .SingleOrDefault();

                TeacherObj tobj = ProtoMapper<Teacher, TeacherObj>.Map(s);
                s.Courses.ForEach(course => tobj.CourseIds.Add(course.Id));
